...
void loop() {
  ...
  x = x - X_DEFAULT;
  y = y - Y_DEFAULT;    //Hiermit habe ich nun ein "normales" Koordinatensystem
...
//  yPositveAbsolute = makeMinimumZero(yPositveAbsolute);
  ...
  int redDiagonal = makeMinimumZero(x);
  int greenDiagonal = makeMinimumZero(round((0.634 * y) + (0.366 * x)));
  int blueDiagonal = makeMinimumZero(-1 * (round((0.634 * y) + (0.366 * x))));
